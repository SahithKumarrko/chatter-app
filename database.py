...
def insert_new(from_u,to_u,msg,con):
    cur=con.cursor()
    d=datetime.now(tz).strftime('%d-%m-%Y %I:%M:%S %p')
...
def retrieve_onload(f_u,con):
    ...
    for da in data4:
        data3.append(da)
    data4=cur.execute(f"select to_u,date_msg from new_messages where from_u='{f_u}' group by to_u")
    data4=data4.fetchall()
    # print(data4)
    for da in data4:
        data3.append(da)
...
# insert_global("Chat Bot","It works",con)

# insert_new("sahith","kumar","hi",con)
# insert_new("sahith","kumar","hijjjj",con)
# print(retrieve_onload("sahith",con))
# insert_new("sahith","kumar3","hiiii",con)
# print(retrieve_onload("sahith",con))